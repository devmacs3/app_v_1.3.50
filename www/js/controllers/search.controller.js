(function() {
    ...
    SearchController.$inject = ['$scope', '$state', 'ToastService', 'Persistence',
        'GlobalService', '$cordovaBarcodeScanner', 'LocalizationService', 'FormService', 'SearchService', '$stateParams'
    ];

    function SearchController($scope, $state, ToastService, Persistence,
        GlobalService, $cordovaBarcodeScanner, LocalizationService, FormService, SearchService, $stateParams) {
        var vm = this;
        vm.results = [];
        vm.searchableTypes = [];
        ...
        function activate() {
            ...
            SearchService.getSearchableFormTypes(function(formTypes) {
                vm.searchableFormTypes = [];
                vm.searchableFormTypes = formTypes;
        ...
        vm.doSearch = function(limit) {
            ...
            if (vm.ResultsCache[vm.status + '_' + vm.searchTerm + '_' + vm.fixedFormType] !== undefined) {
                ...
                if (vm.searchFormType) {
                    options.filters.push({ leftOperand: "form.type", operator: "=", rightOperand: vm.searchFormType });
                }
